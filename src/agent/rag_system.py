...
class RagSystem:
    ...
    def ask(self, question, top_k=4, template_mode="default", return_retrieval=False):
        assert template_mode in [
            "default",
            "multi_choice",
            "TF_answer",
            "multi_choice_explain",
            "single_choice",
        ], "Error: template_mode should be default, multi-choice or TF-answer."
        retrieval, simlarity, indices = self.database.retrieve_with_similarity(
            question, top_k=top_k, return_index=True
        )
        ...
        chat_template = rag_chat_template(
            concatenated_retrieval, question, template_mode
        )
        if not return_retrieval:
            return self.llm.generate(chat_template), [
                {"text": t, "simlarity": float(s)} for t, s in zip(retrieval, simlarity)
            ]
        else:
            return (
                self.llm.generate(chat_template),
                [
                    {"text": t, "simlarity": float(s)}
                    for t, s in zip(retrieval, simlarity)
                ],
                concatenated_retrieval,
            )
    ...
    def get_column_lengths(self):
        self.column_lengths = {
            key: len(value) for key, value in self.database.columns.items()
        }
...
class reranker_RagSystem(rag_framework.RagSystem):
    def __init__(
        ...
        llm: HfWrapper,
        reranker,
        format_rerank: Callable = lambda x: [
            a + b for a, b in zip(x["question"], x["answer"])
        ],
        format_retrieval: Callable = lambda x: [
            a + b for a, b in zip(x["question"], x["answer"])
        ],
        ...
        super().__init__(
            ...
            llm,
            reranker=reranker,
            format_rerank=format_rerank,
            format_retrieval=format_retrieval,
        )
        self.ask_history = dict()
        self.simp_coverage = None
    ...
    def ask(
    ...
    ) -> Union[Iterator[str], Tuple[str, List[Dict[str, float]], List[str]]]:
        ...
        retrieval, scores, indices = self.fetch(
            query, n_retrieval, n_rerank, return_index=True
        )
        ...
        answer = (
            ...
            if not stream
            else self.stream(docs, query, template_mode)
        )
        ...
        self.conversation_history.append(
            {
                "role": "assistant",
                "content": answer if isinstance(answer, str) else "".join(answer),
            }
        )

        if not return_retrieval:
            return answer, [
                {"similarity": float(s), "indices": indices} for s in scores
            ]
        else:
            return (
                answer,
...
class DPRerankerRagSystem(rag_framework.DPRagSystem):
    ...
    def ask(
        ...
        # assert template_mode in ['default', 'multi_choice', 'TF_answer', 'multi_choice_explain', 'single_choice', 'ask', 'defense'], 'Error: template_mode should be default, multi-choice or TF-answer, single_choice, ask, defense.'
        retrieval, scores, indices = self.fetch(
            query, epsilon, p, alpha, min_tau_bins, n_rerank, return_index=True
        )
        # update the ask history
        ...
        if stream:
            ...
            if not return_retrieval:
                return self.generate(docs, query, template_mode), [
                    {"similarity": float(s), "indices": indices} for s in scores
                ]
            else:
                return (
                    self.generate(docs, query, template_mode),
                    [{"similarity": float(s), "indices": indices} for s in scores],
                    docs,
                )
