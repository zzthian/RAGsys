...
def rag_chat_template(
    ...
    prompt_dict = {
        ...
        "strict": "You are an assistant for question-answering tasks. Use the following pieces of retrieved context to answer the question. \n If you don't know the answer or the retrieved context unrelated to the question, say that you don't know. \n\n {}",
        "single_choice": (
    ...
    assert (
        prompt_mode in prompt_dict.keys()
    ), "Error: prompt_mode should one of: {}".format(", ".join(prompt_dict.keys()))
    ...
    return [
        {
            "role": "system",
            "content": "You are a helpful assistant for question-answering tasks. You should only answer with the provided retrievals and in context of the entire conversation history. If you don't know, please say 'I don't know.'.",
        },
        *conversation_history,
        {
            "role": "user",
            "content": "The retrievals are : "
...
class HfWrapper(LlmInterface):
    ...
    def __init__(self, model_name: str, device="cuda"):
        ...
        self.stopping_criteria = transformers.StoppingCriteriaList(
            [StopOnToken(self.model.config.eos_token_id)]
        )
    ...
    def generate(self, chat_template: List[Dict[str, str]]):
        ...
        return self.tokenizer.decode(
            new_token_ids[0][model_input.size(1) :], skip_special_tokens=True
        )
...
class OpenAiWrapper(LlmInterface):
    ...
    def generate(self, chat_template: List[Dict[str, str]], temperature: float = 1.0):
        completion = self.client.chat.completions.create(
            model=self.model_name, messages=chat_template, temperature=temperature
        )
        return completion.choices[0].message.content

    def stream(self, chat_template: List[Dict[str, str]]):
        completion = self.client.chat.completions.create(
            model=self.model_name, messages=chat_template, stream=True
        )
