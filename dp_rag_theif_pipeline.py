...
from src.agent.rag_system import RagSystem, reranker_RagSystem, DPRerankerRagSystem
from src.agent.attacker import  is_refusal_response, check_unanswerable, check_idontknow
from src.rag_framework import RagDatabase, HfWrapper, OpenAiWrapper, transpose_json, DPRagDatabase, text_similarity, find_unsimilar_texts
...
def parse_args():
    ...
    parser.add_argument(
        "--llm_choice", "-llm", type=str, default="deepseek",
    ...
    parser.add_argument(
        "--defense_policy", type=str, default="no_defense",
...
def init_rag(dataset_path, llm_choice="llama", emb_modle_choice="mpnet", use_dp:bool=False, use_reranker:bool=False):
    ...
    if not os.path.exists(defaul_rag_save_name):
        ...
        rag_database = DPRagDatabase.from_texts(
            embedding_model,
            rag_dataset["input"],
        ...
        rag_database = DPRagDatabase.load(defaul_rag_save_name, embedding_model)
    
    Rag_system = DPRerankerRagSystem(rag_database, embedding_model, chat_llm, reranker)
...
def main():
    ...
    # --- experiment setting --- #
    max_extraction_iteration = 64
    
    # --- extraction mode setting --- #
    defense_on = (args.defense_policy!='no_defense')
    defense_policy = args.defense_policy # 'input_filter' or 'output_filter' or 'intention_detection' or 'hybrid' or 'no_defense
    ...
    n_retrieval = 16
    n_rerank = 4
    p = n_retrieval/max(rag_system.get_column_lengths().values())
    epsilon = 0.5
    ...
    result_record_name = f"/home/guest/data/IKEA/main_record/dp_IKEA/([{custom_label}]-[rt_{n_retrieval}_rr_{n_rerank}]-[{embedding_model_name.split('/')[-1]}]-[{llm_choice}]-[{dataset_label}]) [DP]-[{defense_policy}]-[{attack_baseline}]-[{max_extraction_iteration}] mutation_pipeline_result.jsonl"
    ...
    with tqdm(total=max_extraction_iteration) as pbar:
        while count < max_extraction_iteration:          
            ...
            if not (defense_on and detector_detection):
                ...
                answer, similarity_list, retrieval = rag_system.ask(question, epsilon=epsilon, p=p, alpha=1, min_tau_bins=100, n_rerank=n_rerank, template_mode= template_mode, return_retrieval=True)
            ...
            origin_answer_flag = check_idontknow(answer, 50) or illegal_flag
